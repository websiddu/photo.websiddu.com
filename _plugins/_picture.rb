  ...
  class Picture < Liquid::Tag
    ...
    def render(context)
      ...
      instance.each { |key, source|
        ...
        instance[key][:ratio] = get_ratio(source, site.source, settings['source'])
      }
      ...
      if settings['markup'] == 'picture'
        ...
        orig_ratio = get_ratio(instance['source_default'], site.source, settings['source'])
        ...
        picture_tag = "<picture class='column is-#{html_attr['size']}' data-ratio='#{orig_ratio.to_s}'>\n"\
                      "#{source_tags}"\
                      "#{markdown_escape * 4}<img src='images/loader.svg' data-src=\"#{url}#{instance['source_default'][:generated_src]}\" #{html_attr_string}>\n"\
    ...
    def get_ratio(instance, site_source, image_source)
